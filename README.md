...
A Java GUI implementation of the traditional Ethiopian board game Gebeta (Mancala), featuring an AI opponent with time bounded iterative deepening, a fully unit tested rules engine, and a Gradle build with continuous integration. The interface is themed around Habesha (Ethiopian) culture, with warm coffee and gold tones over a backdrop of elders playing gebeta.

**[Play Gebeta in your browser](https://yabtesfu.github.io/Gebeta-Game/)** requires no download or account.

![Gameplay demo showing animated stone sowing](docs/demo.gif)
...
![Main menu](docs/screenshot-intro.png)

![Game screen](docs/screenshot-game.png)
...
release is not notarized by Apple, macOS may require a right click followed by **Open**
the first time.

### Build from source

Build a self contained, runnable jar and launch it. No IDE is required, just a JDK:
...
* **Persistent records:** Versus computer wins, losses, and draws, along with local Player 1 / Player 2
...
* **Habesha themed UI:** A cohesive traditional Ethiopian look with a coffee and gold palette,
...
3. **Distribution:** Stones are distributed one by one into subsequent pits counterclockwise
